...
use postfix_admin_db::{
    mysql_transaction, DbError, MysqlAdminRepository, MysqlAliasDomainRepository,
    MysqlAliasRepository, MysqlAppPasswordRepository, MysqlDkimRepository, MysqlDomainRepository,
    MysqlFetchmailRepository, MysqlLogRepository, MysqlMailboxRepository, MysqlVacationRepository,
};

#[tokio::test]
async fn mysql_domain_create_valid_returns_response() {
    let (_container, pool) = common::setup_mysql().await;
    let repo = MysqlDomainRepository::new(pool);
...
#[tokio::test]
async fn mysql_domain_find_by_name_existing_returns_some() {
    let (_container, pool) = common::setup_mysql().await;
    let repo = MysqlDomainRepository::new(pool);
...
#[tokio::test]
async fn mysql_domain_update_changes_description() {
    let (_container, pool) = common::setup_mysql().await;
    let repo = MysqlDomainRepository::new(pool);
...
#[tokio::test]
async fn mysql_domain_delete_removes_domain() {
    let (_container, pool) = common::setup_mysql().await;
    let repo = MysqlDomainRepository::new(pool);
...
#[tokio::test]
async fn mysql_domain_count_returns_correct_value() {
    let (_container, pool) = common::setup_mysql().await;
    let repo = MysqlDomainRepository::new(pool);
...
#[tokio::test]
async fn mysql_admin_create_and_find_by_username() {
    let (_container, pool) = common::setup_mysql().await;
    let repo = MysqlAdminRepository::new(pool);
...
#[tokio::test]
async fn mysql_mailbox_create_and_find_by_domain() {
    let (_container, pool) = common::setup_mysql().await;
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let repo = MysqlMailboxRepository::new(pool);
...
#[tokio::test]
async fn mysql_mailbox_update_changes_name() {
    let (_container, pool) = common::setup_mysql().await;
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let repo = MysqlMailboxRepository::new(pool);
...
#[tokio::test]
async fn mysql_alias_create_and_find() {
    let (_container, pool) = common::setup_mysql().await;
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let repo = MysqlAliasRepository::new(pool);
...
#[tokio::test]
async fn mysql_alias_update_changes_goto() {
    let (_container, pool) = common::setup_mysql().await;
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let repo = MysqlAliasRepository::new(pool);
...
#[tokio::test]
async fn mysql_alias_domain_create_and_find() {
    let (_container, pool) = common::setup_mysql().await;
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let repo = MysqlAliasDomainRepository::new(pool);
...
#[tokio::test]
async fn mysql_vacation_upsert_and_find() {
    let (_container, pool) = common::setup_mysql().await;
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let repo = MysqlVacationRepository::new(pool);
...
#[tokio::test]
async fn mysql_dkim_create_key_and_signing() {
    let (_container, pool) = common::setup_mysql().await;
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let repo = MysqlDkimRepository::new(pool);
...
#[tokio::test]
async fn mysql_fetchmail_create_and_find() {
    let (_container, pool) = common::setup_mysql().await;
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let mailbox_repo = MysqlMailboxRepository::new(pool.clone());
    let repo = MysqlFetchmailRepository::new(pool);
...
#[tokio::test]
async fn mysql_log_create_and_find() {
    let (_container, pool) = common::setup_mysql().await;
    let repo = MysqlLogRepository::new(pool);
...
#[tokio::test]
async fn mysql_app_password_create_and_find() {
    let (_container, pool) = common::setup_mysql().await;
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let mailbox_repo = MysqlMailboxRepository::new(pool.clone());
    let repo = MysqlAppPasswordRepository::new(pool);
...
#[tokio::test]
async fn mysql_transaction_commit_persists_data() {
    let (_container, pool) = common::setup_mysql().await;

    mysql_transaction(&pool, |conn| {
        Box::pin(async move {
            sqlx::query("INSERT INTO domain (domain) VALUES (?)")
                .bind("tx-commit.com")
                .execute(&mut *conn)
                .await?;
            Ok(())
...
#[tokio::test]
async fn mysql_transaction_error_rolls_back() {
    let (_container, pool) = common::setup_mysql().await;

    let result: Result<(), DbError> = mysql_transaction(&pool, |conn| {
        Box::pin(async move {
            sqlx::query("INSERT INTO domain (domain) VALUES (?)")
...
#[tokio::test]
async fn mysql_transaction_multiple_operations_atomic() {
    let (_container, pool) = common::setup_mysql().await;
    ...
    mysql_transaction(&pool, |conn| {
        Box::pin(async move {
            sqlx::query("INSERT INTO domain (domain) VALUES (?)")
                ...
                .execute(&mut *conn)
                .await?;
            sqlx::query("INSERT INTO alias (address, goto, domain) VALUES (?, ?, ?)")
    ...
    let domain_repo = MysqlDomainRepository::new(pool.clone());
    let alias_repo = MysqlAliasRepository::new(pool);
